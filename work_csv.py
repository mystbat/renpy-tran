...
try:
    ...
    start_time = time.time()
    # 读取CSV消息文本文件
    with open(os.path.join(current_directory, 'csv_message.txt'), 'r', encoding='utf-8') as message_file:
        user_message = message_file.read().strip()

    # 读取上下文文件
    context_file_path = os.path.join(current_directory, 'csv_top_message.json')
    context_messages = []
...
try:
    ...
    if response.status_code == 200:
        ...
        # 只保存到csv.txt（覆盖方式）
        csv_path = os.path.join(response_dir, 'csv.txt')
        with open(csv_path, 'w', encoding='utf-8') as f:
            f.write(model_response)
            f.write('\n')  # 添加换行符

        print(f"模型回答已保存到文件: {csv_path}")
    else:
        # 如果响应不成功，不保存任何内容
